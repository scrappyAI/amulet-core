...
pub struct Event {
    ...
    /// Additional fields not defined in this version of the Event struct.
    /// This field is used to preserve unknown future fields bit-exact when an event
    /// is deserialized and then re-serialized (e.g., when relayed by a replica).
    /// This ensures forward compatibility as the event schema evolves.
    /// The keys are field names, and values are their raw byte representation.
    pub additional_fields: Option<BTreeMap<String, Vec<u8>>>,
...
//     This struct uses the `additional_fields` member to support this requirement.
//     The chosen serialization format and its implementation must populate this field
//     during deserialization if unknown fields are encountered and include its contents
